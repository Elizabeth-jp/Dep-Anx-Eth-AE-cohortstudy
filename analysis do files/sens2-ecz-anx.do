...
TASK:					sensitivity analysis 2 - using alternative code lists to
						identify problems with sleep_all
...
DATASETS CREATED:		sens2-cohort-anx-ecz
						sens2-ecz-anx.xls
*******************************************************************************/
/*******************************************************************************
HOUSEKEEPING
...
*save the dataset
save "${pathCohort}/sens2-cohort-anx-ecz-exposed", replace

/*******************************************************************************
#1.2: Deal with time-updated variables
*******************************************************************************/
*merge in time-updated variables dataset
merge 1:m patid using "${pathCohort}/sens2-ecz-anx-time-updated-variables"
...
merge m:1 patid using "${pathCohort}/sens2-cohort-anx-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens2-cohort-anx-ecz-unexposed", replace
...
merge m:1 patid using "${pathCohort}/sens2-cohort-anx-ecz-unexposed"
...
*******************************************************************************/
append using "${pathCohort}/sens2-cohort-anx-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens2-cohort-anx-ecz-main", replace

*delete interim datasets
erase "${pathCohort}/sens2-cohort-anx-ecz-exposed.dta"
erase "${pathCohort}/sens2-cohort-anx-ecz-unexposed.dta"
...
merge m:1 patid using "${pathIn}/variables-ecz-sleep-all.dta"
drop if _merge==2
drop _merge
*(patid from using file)

*save the dataset
save "${pathCohort}/sens2-cohort-anx-ecz", replace
...
label var sleep_all "sleep_all problems 0=No, 1=Yes"
...
/*
--------------------------------------------------------------------------
  1,322,888  total observations
         17  observations end on or before enter()
--------------------------------------------------------------------------
  1,322,871  observations remaining, representing
    999,430  subjects
     58,563  failures in single-failure-per-subject data
  3,868,457  total analysis time at risk and under observation
...
*******************************************************************************/
*recode those with missing harmful alcohol, sleep_all, steroids and anxiety as not having them
recode harmfulalcohol (.=0)
recode asthma (.=0)
recode sleep_all (.=0)
recode anxiety (.=0)
...
use "${pathCohort}/sens2-cohort-anx-ecz", clear
...
comorbidity burden,asthma, sleep_all problems, smoking status,high dose steroids, harmful alcohol use, BMI
*******************************************************************************/
*open analysis dataset
use "${pathCohort}/sens2-cohort-anx-ecz", clear
...
*run analysis in white ethnic group
display in red "**************** White ********************************"
stcox i.exposed i.calendarperiod i.carstairs_deprivation i.cci i.asthma i.sleep_all i.harmfulalcohol i.smokstatus i.bmi_cat i.steroids if ethnicity==1, strata(setid) level(95) base
matrix table = r(table) 
gethr table ethWE

*run analysis in minority ethnic group
display in red "**************** Minority ethnic ********************************"
stcox i.exposed i.calendarperiod i.carstairs_deprivation i.cci i.asthma i.sleep_all i.harmfulalcohol i.smokstatus i.bmi_cat i.steroids if ethnicity==2, strata(setid) level(95) base
matrix table = r(table) 
gethr table ethME

*run interaction analysis 
display in red "**************** ethnicity interaction ********************************"
stcox i.exposed##i.ethnicity i.calendarperiod i.carstairs_deprivation i.cci i.asthma i.sleep_all i.harmfulalcohol i.smokstatus i.bmi_cat i.steroids, strata(setid) level(95) base
matrix table = r(table) 
...
* create excel file
putexcel set "${pathResults}/sens2-ecz-anx-regression.xlsx", sheet(confounder) modify
...
* create excel file
putexcel set "${pathResults}/sens2-ecz-anx-regression.xlsx", sheet(mediator) modify
