/*******************************************************************************
DO FILE NAME:			appendix-ecz-dep-missingChar.do

AUTHOR:					Elizabeth Adesanya
						

VERSION:				v1
DATE VERSION CREATED: 	20/09/2022

TASK:					Create a table for the appendix including data for:
						1. the whole cohort (with eczema and without eczema)
						2. confounder-adjusted model (model1 - adjusts for carstairs and calendar period)
						3. individuals with missing carstairs
						4. mediator-adjusted model (model4 - adjusts for other mediators including 
...
DATASETS CREATED:		appendix-ecz-dep-missingChar.xls
...
global filename "appendix-ecz-dep-missingChar"
...
foreach sample in all model1 noCar model4 noBMI noSmok {
	...
	*Label variables
	...
	label var sex "Sex"
	label var carstairs "Carstairs index quintile"
	...
	***************************************************************************/
	/*--------------------------------------------------------------------------
		...
		----------------------------------------------------------------------*/
		local bincv " "asthma" "harmfulalcohol" "sleep" "sex" "
		foreach group in exp unexp {
		    foreach cv in `bincv' {
		...
		--------------------------------------------------------------------------*/	
		local multicv " "carstairs" "ethnicity" "smokstatus" "bmi_cat" "cci" "
...
*******************************************************************************/
* create excel file
putexcel set "${pathResults}/appendix-ecz-dep-missingChar.xlsx", replace

* set row count variable
local rowcount=1

* Table title
putexcel A`rowcount'="Table x. Characteristics of the study population at cohort entry, for: the overall cohort, individuals included in the model additionally adjusting for potential confounders (i.e. individuals with no missing Carstairs deprivation data), individuals with missing Carstairs data, individuals included in the model additionally adjusting for potential mediators (i.e.individuals with no missing BMI or smoking status data), and for individuals with missing BMI or smoking status." 
...
levelsof carstairs, local(levels)
foreach j of local levels {
    putexcel A`rowcount'="`: label (carstairs) `j''" // use variable label for row caption
	foreach pop in all model1 noCar model4 noBMI noSmok {
		forvalues i=1/2 {
			local col`i' : word `i' of ``pop'' 
		} /*end forvalues i=1/2*/ 
		putexcel `col1'`rowcount'="${`pop'_carstairs`j'_exp}"
		putexcel `col2'`rowcount'="${`pop'_carstairs`j'_unexp}"
		
		*deal with missing information
		
		if "`pop'"=="model1" & `j'==6 { // for missing carstairs in model1
		...
		if "`pop'"=="model4" & `j'==6 { // for missing carstairs in model4
			putexcel `col1'`rowcount'="n/a"
			putexcel `col2'`rowcount'="n/a"
		} /*end if "`pop'"=="model4" & `j'==6 */
...
foreach pop in all model1 noCar model4 noBMI noSmok {
	...
	putexcel `col1'`rowcount'="${`pop'_asthma_exp}"
	putexcel `col2'`rowcount'="${`pop'_asthma_unexp}"
}/*end foreach pop in all model1 noCar model4 noBMI noSmok*/
local ++rowcount
