...
TASK:					sensitivity analysis 4 - not censoring at alternative
						diagnosis (severe mental illness) in anxiety cohort
...
DATASETS CREATED:		sens4-cohort-anx-ecz
						sens4-ecz-anx.xls
*******************************************************************************/
/*******************************************************************************
HOUSEKEEPING
...
drop earlyanxiety anxiety anxiety_date


*save the dataset
save "${pathCohort}/sens4-cohort-anx-ecz-exposed", replace

/*******************************************************************************
#1.2: Deal with time-updated variables
*******************************************************************************/
*merge in time-updated variables dataset
merge 1:m patid using "${pathCohort}/sens4-ecz-anx-time-updated-variables"
...
drop if date!=. & date>enddate_incanx

*merge in exposed cohort 
merge m:1 patid using "${pathCohort}/sens4-cohort-anx-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens4-cohort-anx-ecz-unexposed", replace
...
merge m:1 patid using "${pathCohort}/sens4-cohort-anx-ecz-unexposed"
...
*******************************************************************************/
append using "${pathCohort}/sens4-cohort-anx-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens4-cohort-anx-ecz", replace

*delete interim datasets
erase "${pathCohort}/sens4-cohort-anx-ecz-exposed.dta"
erase "${pathCohort}/sens4-cohort-anx-ecz-unexposed.dta"
...
replace end=enddate_incanx+1 if end==. & anxiety==1 
*will end the day after anxiety outcome - prevents stata from not including multiple observations

replace end=enddate_incanx if end==.
...
/*
--------------------------------------------------------------------------
  1,312,956  total observations
         17  observations end on or before enter()
--------------------------------------------------------------------------
  1,312,939  observations remaining, representing
    999,430  subjects
...
use "${pathCohort}/sens4-cohort-anx-ecz", clear
...
* create excel file
putexcel set "${pathResults}/sens4-ecz-anx-regression.xlsx", sheet(confounder) modify
...
* create excel file
putexcel set "${pathResults}/sens4-ecz-anx-regression.xlsx", sheet(mediator) modify
