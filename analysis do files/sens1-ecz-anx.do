...
DATASETS CREATED:		sens1-cohort-anx-ecz
						sens1-ecz-anx.xls
*******************************************************************************/
/*******************************************************************************
HOUSEKEEPING
...
merge 1:1 patid using "${pathIn}/outcome-ecz-anx-all.dta"
...
drop earlyanxiety anxiety_all _merge anxiety_date
...
*save the dataset
save "${pathCohort}/sens1-cohort-anx-ecz-exposed", replace

/*******************************************************************************
#1.2: Deal with time-updated variables
*******************************************************************************/
*merge in time-updated variables dataset
merge 1:m patid using "${pathCohort}/sens1-ecz-anx-time-updated-variables"
...
merge m:1 patid using "${pathCohort}/sens1-cohort-anx-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens1-cohort-anx-ecz-unexposed", replace
...
merge m:1 patid using "${pathCohort}/sens1-cohort-anx-ecz-unexposed"
...
*******************************************************************************/
append using "${pathCohort}/sens1-cohort-anx-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens1-cohort-anx-ecz-main", replace

*delete interim datasets
erase "${pathCohort}/sens1-cohort-anx-ecz-exposed.dta"
erase "${pathCohort}/sens1-cohort-anx-ecz-unexposed.dta"
...
unique patid 
*Total number of people is 968,208

unique patid if exposed==1
*Number of unique values of patid is 237,039  

unique patid if exposed==0
*Number of unique values of patid is 747,742
...
*save the dataset
save "${pathCohort}/sens1-cohort-anx-ecz", replace
...
replace end=enddate_inccensor+1 if end==. & anxiety_all==1 
...
*******************************************************************************/
stset end, fail(anxiety_all==1) origin(realyob) enter(indexdate) id(patid) scale(365.25) 

/*
--------------------------------------------------------------------------
  1,270,584  total observations
         17  observations end on or before enter()
--------------------------------------------------------------------------
  1,270,567  observations remaining, representing
    968,208  subjects
     61,615  failures in single-failure-per-subject data
  3,739,339  total analysis time at risk and under observation
...
use "${pathCohort}/sens1-cohort-anx-ecz", clear
...
* create excel file
putexcel set "${pathResults}/sens1-ecz-anx-regression.xlsx", sheet(confounder) modify
...
* create excel file
putexcel set "${pathResults}/sens1-ecz-anx-regression.xlsx", sheet(mediator) modify
