...
TASK:					sensitivity analysis 1 - using alternative code lists to
						define depression
...
DATASETS CREATED:		sens1-cohort-dep-ecz
						sens1-ecz-dep.xls
*******************************************************************************/
/*******************************************************************************
HOUSEKEEPING
...
merge 1:1 patid using "${pathIn}/outcome-ecz-dep-all.dta"
...
drop earlydepression depression_all _merge depression_date
...
*save the dataset
save "${pathCohort}/sens1-cohort-dep-ecz-exposed", replace

/*******************************************************************************
#1.2: Deal with time-updated variables
*******************************************************************************/
*merge in time-updated variables dataset
merge 1:m patid using "${pathCohort}/sens1-ecz-dep-time-updated-variables"
...
merge m:1 patid using "${pathCohort}/sens1-cohort-dep-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens1-cohort-dep-ecz-unexposed", replace
...
merge m:1 patid using "${pathCohort}/sens1-cohort-dep-ecz-unexposed"
...
*******************************************************************************/
append using "${pathCohort}/sens1-cohort-dep-ecz-exposed"
...
*save the dataset
save "${pathCohort}/sens1-cohort-dep-ecz-main", replace

*delete interim datasets
erase "${pathCohort}/sens1-cohort-dep-ecz-exposed.dta"
erase "${pathCohort}/sens1-cohort-dep-ecz-unexposed.dta"
...
unique patid 
*Total number of people is 844,279

unique patid if exposed==1
*Number of unique values of patid is 214,326  

unique patid if exposed==0
*Number of unique values of patid is 643,543
...
*save the dataset
save "${pathCohort}/sens1-cohort-dep-ecz", replace
...
replace end=enddate_inccensor+1 if end==. & depression_all==1 
...
*******************************************************************************/
stset end, fail(depression_all==1) origin(realyob) enter(indexdate) id(patid) scale(365.25) 

/*
--------------------------------------------------------------------------
  1,083,177  total observations
         14  observations end on or before enter()
--------------------------------------------------------------------------
  1,083,163  observations remaining, representing
    844,279  subjects
     66,143  failures in single-failure-per-subject data
  3,211,416  total analysis time at risk and under observation
...
use "${pathCohort}/sens1-cohort-dep-ecz", clear
...
* create excel file
putexcel set "${pathResults}/sens1-ecz-dep-regression.xlsx", sheet(confounder) modify
...
* create excel file
putexcel set "${pathResults}/sens1-ecz-dep-regression.xlsx", sheet(mediator) modify
